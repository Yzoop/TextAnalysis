...
def print_metrics(model, label_binarizer, train_x, test_x, train_y, test_y):
...
if __name__ == "__main__":
    ...
    text_vectors, label_vectors, label_binarizer = convert_data_to_vectors(texts, labels, return_binarizer=True)
    train_x, test_x, train_y, test_y = train_test_split(text_vectors,
                                                        label_vectors,
                                                        test_size=0.2,
                                                        random_state=17)
    multiclass_classifier = train_classifier(train_x, train_y)
    print_metrics(multiclass_classifier, label_binarizer, train_x, test_x, train_y, test_y)
